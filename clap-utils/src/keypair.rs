            ...
            parse_signer_source(&self.path)
                ...
                .map_err(|_| {
                    std::io::Error::new(
                        ...
                        format!(
                        "No const_data signer found, run \"station-svm-keygen new -o {}\" to create a new one",
                        self.path
                    ),
                    )
                })?;
    ...
    match kind {
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                format!("could not read keypair file \"{path}\". Run \"station-svm-keygen new\" to create a keypair file: {e}"),
            )
            .into()),
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                ...
                format!(
                    ...
                    Run \"station-svm-keygen new\" to create a keypair file: {e}"
                ),
            )
            .into()),
