...
pub struct TestValidatorGenesis {
    ...
    // (JsonRpc, JsonRpcPubSub), None == random ports
...
impl TestValidator {
    ...
    fn start(
        ...
        let mut validator_config = ValidatorConfig {
            ...
            accounts_hash_interval_slots: 2000,
            account_paths: vec![
                create_accounts_run_and_snapshot_dirs(ledger_path.join("accounts"))
                    .unwrap()
                    .0,
            ],
            run_verification: true, // Skip PoH verification of ledger on startup for speed
            snapshot_config: SnapshotConfig {
                full_snapshot_archive_interval_slots: 2000,
