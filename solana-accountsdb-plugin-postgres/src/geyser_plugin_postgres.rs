...
use {
    ...
    serde_json,
    solana_entry::entry::{Entry, UntrustedEntry},
    solana_geyser_plugin_interface::geyser_plugin_interface::{
        GeyserPlugin, GeyserPluginError, ReplicaAccountInfoVersions, ReplicaBlockInfoVersions,
        ReplicaTransactionInfoVersions, Result, SlotStatus, ReplicaEntryInfoVersions
    },
...
pub struct GeyserPluginPostgresConfig {
    ...
    /// The port number of the PostgreSQL database, the const_data is 5432
    pub port: Option<u16>,
    ...
    /// Controls whether to index the token owners. The const_data is false
    pub index_token_owner: Option<bool>,

    /// Controls whetherf to index the token mints. The const_data is false
...
impl GeyserPlugin for GeyserPluginPostgres {
    ...
    /// maintains a PostgreSQL connection to the server. The const_data is '10'.
    /// * "batch_size" optional, specifies the batch size of bulk insert when the AccountsDb is created
    /// from restoring a snapshot. The const_data is '10'.
    /// * "panic_on_db_errors", optional, contols if to panic when there are errors replicating data to the
    /// PostgreSQL database. The const_data is 'false'.
    ...
    fn on_load(&mut self, config_file: &str, is_reload: bool) -> Result<()> {
        ...
        info!(
            "Loading plugin {:?} from config_file {:?} is {:?}",
            self.name(),
            config_file,
            is_reload
        );
    ...
    fn update_account(
        ...
        let mut measure_all = Measure::start("geyser-plugin-db_client-update-account-main");
        match account {
            ...
            ReplicaAccountInfoVersions::V0_0_2(account) => {
                let mut measure_select =
                    Measure::start("geyser-plugin-db_client-update-account-select");
                ...
                inc_new_counter_debug!(
                    "geyser-plugin-db_client-update-account-select-us",
                ...
                match &mut self.client {
                    ...
                    Some(client) => {
                        let mut measure_update =
                            Measure::start("geyser-plugin-db_client-update-account-client");
                        let result = { client.update_account(account, slot, is_startup) };
                        measure_update.stop();

                        inc_new_counter_debug!(
                            "geyser-plugin-db_client-update-account-client-us",
            ...
            // ReplicaAccountInfoVersions::V0_0_3() => {
            //     return Err(GeyserPluginError::Custom(Box::new(
            //         GeyserPluginPostgresError::ReplicaAccountV001NotSupported,
            //     )));
            // }
        ...
        inc_new_counter_debug!(
            "geyser-plugin-db_client-update-account-main-us",
