...
impl SimplePostgresClient {
    ...
    fn bulk_insert_token_index_common(
        ...
        if index_entries.len() == batch_size {
            let mut measure = Measure::start("geyser-plugin-db_client-prepare-index-values");
            ...
            inc_new_counter_debug!(
                "geyser-plugin-db_client-prepare-index-values-us",
                measure.as_us() as usize,
                10000,
                10000
            );

            let mut measure = Measure::start("geyser-plugin-db_client-update-index-account");
            ...
            measure.stop();
            inc_new_counter_debug!(
                "geyser-plugin-db_client-update-index-us",
                measure.as_us() as usize,
                10000,
                10000
            );
            inc_new_counter_debug!(
                "geyser-plugin-db_client-update-index-count",
                batch_size,
                10000,
                10000
            );
