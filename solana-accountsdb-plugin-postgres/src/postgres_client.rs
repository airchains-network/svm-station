...
impl SimplePostgresClient {
    ...
    fn bulk_insert_accounts(&mut self) -> Result<(), GeyserPluginError> {
        if self.pending_account_updates.len() == self.batch_size {
            let mut measure = Measure::start("geyser-plugin-db_client-prepare-values");
            ...
            inc_new_counter_debug!(
                "geyser-plugin-db_client-prepare-values-us",
                measure.as_us() as usize,
                10000,
                10000
            );

            let mut measure = Measure::start("geyser-plugin-db_client-update-account");
            let client = self.client.get_mut().unwrap();
            let result = client
                .client
            ...
            measure.stop();
            inc_new_counter_debug!(
                "geyser-plugin-db_client-update-account-us",
                measure.as_us() as usize,
                10000,
                10000
            );
            inc_new_counter_debug!(
                "geyser-plugin-db_client-update-account-count",
                self.batch_size,
                10000,
                10000
            );
    ...
    fn flush_buffered_writes(&mut self) -> Result<(), GeyserPluginError> {
        ...
        let mut measure = Measure::start("geyser-plugin-db_client-flush-slots-us");
    ...
    fn upsert_slot_status_internal(
        ...
        let slot = slot as i64; // db_client only supports i64
        ...
        let result = match parent {
            ...
            Some(parent) => client.execute(statement, &[&slot, &parent, &status_str,]),
            None => client.execute(statement, &[&slot, &status_str,]),
        };
...
impl PostgresClientWorker {
    ...
    fn do_work(
        ...
        while !exit_worker.load(Ordering::Relaxed) {
            let mut measure = Measure::start("geyser-plugin-db_client-worker-recv");
            let work = receiver.recv_timeout(Duration::from_millis(500));
            measure.stop();
            inc_new_counter_debug!(
                "geyser-plugin-db_client-worker-recv-us",
                measure.as_us() as usize,
                100000,
                100000
            );
...
impl ParallelPostgresClient {
    ...
    pub fn log_entry(&mut self, entry: ReplicaEntryInfoVersions) -> Result<(), GeyserPluginError> {
...
impl SequencePostgresClient {
    pub fn new(config: &GeyserPluginPostgresConfig) -> Result<Self, GeyserPluginError> {
        ...
        let exit_clone = exit_worker.clone();
        let config = config.clone();
        // let smt_tree = Arc::new(RwLock::new(SMT::const_data()));
