...
pub enum RpcCustomError {
    ...
    #[error("PodNotAvailable")]
    PodNotAvailable { pod_number: u64, latest_pod_number: u64 },
    #[error("LatestPodNotAvailable")]
    LatestPodNotAvailable { latest_pod_error: String },
    #[error("NodeUnhealthy")]
...
impl From<RpcCustomError> for Error {
    fn from(e: RpcCustomError) -> Self {
        match e {
            ...
            RpcCustomError::PodNotAvailable { pod_number, latest_pod_number } => Self {
                code: ErrorCode::ServerError(JSON_RPC_SERVER_ERROR_BLOCK_NOT_AVAILABLE),
                message: format!("pod not available for pod number {pod_number} latest pod number is {latest_pod_number}"),
                data: None,
            },
            RpcCustomError::LatestPodNotAvailable { latest_pod_error } => Self {
                code: ErrorCode::ServerError(JSON_RPC_SERVER_ERROR_BLOCK_NOT_AVAILABLE),
                message: format!("{latest_pod_error}"),
                data: None,
            },
