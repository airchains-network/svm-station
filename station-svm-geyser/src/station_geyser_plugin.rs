...
use {
    crate::{
        ...
        seq_txn_settle::{TxnStoreStruct},
        svm_txn_select::SvmTxnSelector,
    },
    log::*,
...
impl GeyserPlugin for GeyserPluginRocksDB {
    ...
    fn on_load(&mut self, config_file: &str, is_bool: bool) -> Result<()> {
        ...
        let config = RocksDBConnection::open("test-ledger/sequencer").unwrap();
        ...
        match &self.client {
            ...
            Some(client) => match transaction_info {
                ReplicaTransactionInfoVersions::V0_0_2(transaction_info) => {
                    ...
                    let _ = TxnStoreStruct::tx_store(client.to_owned(), txn_count, transaction_info, slot);
                }
                _ => {
                    return Err(GeyserPluginError::SlotStatusUpdateError {
