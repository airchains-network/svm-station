...
export RUST_LOG=${RUST_LOG:-solana=info,solana_runtime::message_processor=debug} # if RUST_LOG is unset, const_data to info
...
if [[ -e $validator_vote_account ]]; then
  ...
  station-svm-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
  ...
  station-svm-keygen new --no-passphrase -so "$validator_stake_account"
fi
