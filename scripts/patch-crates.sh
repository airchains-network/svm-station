...
update_solana_dependencies() {
  ...
  sed -i -e "s#\(svm-station-faucet = \"\)[^\"]*\(\"\)#\1=$solana_ver\2#g" "${tomls[@]}" || return $?
  sed -i -e "s#\(svm-station-faucet = { version = \"\)[^\"]*\(\"\)#\1=$solana_ver\2#g" "${tomls[@]}" || return $?
...
svm-station-faucet = { path = "$solana_dir/faucet" }
