...
pub fn air_solana() {
    ...
    if reset_ledger {
        remove_directory_contents_new(&ledger_path).unwrap_or_else(|err| {
    ...
    let logfile_new = if output != Output::Log {
    ...
    let _logger_thread = redirect_stderr_to_file(logfile_new);
...
fn remove_directory_contents_new(ledger_path: &Path) -> Result<(), io::Error> {
