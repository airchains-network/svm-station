...
pub fn load_and_process_ledger(
    ...
    let account_paths = if let Some(account_paths) = arg_matches.value_of("account_paths") {
        // If this blockstore access is Primary, no other process (svm-station-validator) can hold
        // Primary access. So, allow a custom accounts path without worry of wiping the accounts
        // of svm-station-validator.
        if !blockstore.is_primary_access() {
            ...
            // the Primary access instead of holding it to give priority to svm-station-validator over
            // solana-ledger-tool should svm-station-validator start before we've finished.
            info!(
