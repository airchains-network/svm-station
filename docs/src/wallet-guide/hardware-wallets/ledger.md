...
```bash
svm-station-keygen pubkey usb://ledger
```
...
To view any address, use the `svm-station-keygen pubkey` command, as shown below,
...
```bash
svm-station-keygen pubkey usb://ledger
svm-station-keygen pubkey usb://ledger?key=0
svm-station-keygen pubkey usb://ledger?key=1
svm-station-keygen pubkey usb://ledger?key=2
```
...
```bash
~$ svm-station-keygen pubkey usb://ledger?key=42
