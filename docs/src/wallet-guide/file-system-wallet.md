...
Use Solana's command-line tool `station-svm-keygen` to generate keypair files. For
example, run the following from a command-line shell:

```bash
mkdir ~/my-solana-wallet
station-svm-keygen new --outfile ~/my-solana-wallet/my-keypair.json
```
...
```bash
station-svm-keygen pubkey ~/my-solana-wallet/my-keypair.json
```
...
```bash
station-svm-keygen verify <PUBKEY> ~/my-solana-wallet/my-keypair.json
```
