...
Use Solana's command-line tool `stationsvm-keygen` to generate keypair files. For
example, run the following from a command-line shell:

```bash
mkdir ~/my-solana-wallet
stationsvm-keygen new --outfile ~/my-solana-wallet/my-keypair.json
```
...
```bash
stationsvm-keygen pubkey ~/my-solana-wallet/my-keypair.json
```
...
```bash
stationsvm-keygen verify <PUBKEY> ~/my-solana-wallet/my-keypair.json
```
