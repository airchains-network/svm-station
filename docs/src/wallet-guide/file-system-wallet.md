...
Use Solana's command-line tool `svm-station-keygen` to generate keypair files. For
example, run the following from a command-line shell:

```bash
mkdir ~/my-solana-wallet
svm-station-keygen new --outfile ~/my-solana-wallet/my-keypair.json
```
...
```bash
svm-station-keygen pubkey ~/my-solana-wallet/my-keypair.json
```
...
```bash
svm-station-keygen verify <PUBKEY> ~/my-solana-wallet/my-keypair.json
```
