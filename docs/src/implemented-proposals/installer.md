...
```bash
$ svm-station-keygen new -o update-manifest.json  # <-- only generated once, the public key is shared with users
...
$ svm-station-keygen ...  # <-- runs the latest svm-station-keygen
$ solana-install run svm-station-validator ...  # <-- runs a validator, restarting it as necessary when an update is applied
```
