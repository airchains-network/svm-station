...
From within the Solana CLI, you can execute the `svm-station-validator` command with
...
Assuming you are ready to restart, you may use the `svm-station-validator exit`
...
your `svm-station-validator` binary to your specific hardware.
...
the newly installed version is in use. Use `svm-station-validator exit` to restart
...
`svm-station-validator` command:

```
--no-snapshot-fetch
```

If you use this flag with the `svm-station-validator` command, make sure that you run
...
running the `svm-station-validator` software. How much SOL you should store there is
