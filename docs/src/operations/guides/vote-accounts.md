...
smooth transition of the vote signing, `svm-station-validator` allows the
...
1. Create the new validator identity keypair,
   `svm-station-keygen new -o ~/new-validator-keypair.json`.
...
2. Create the new vote authority keypair,
   `svm-station-keygen new -o ~/new-vote-authority.json`.
