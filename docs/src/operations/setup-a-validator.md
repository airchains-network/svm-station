...
Refer to `svm-station-validator --help` for more information on what each flag is
...
The script should execute the `svm-station-validator` process. In a new terminal
...
You should see a line in the output that includes `svm-station-validator` with all
